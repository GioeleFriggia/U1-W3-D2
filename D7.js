...
function concatenaStringhe(str1, str2) {
  ...
  const risultato = (parte1 + parte2).toUpperCase();
  console.log(risultato);
}
concatenaStringhe("Ciao", "Epicode");

/* ESERCIZIO 2 (for)
  Scrivi una funzione che torni un array di 10 elementi; ognuno di essi deve essere un valore random compreso tra 0 e 100 (incluso).
*/
function arrayDiNumeriCasuali() {
  const arrayCasuale = [];
  for (let i = 0; i < 10; i++) {
...
*/
function sommaArray(arrayNumericoparams) {
  let somma = 0;
  arrayNumericoparams.forEach((numero) => {
    somma += numero;
  });
  return somma;
}
...
/* ESERCIZIO 5 (reduce)
  Scrivi una funzione per sommare i numeri contenuti in un array
*/
function sommaArrayConReduce(arrayNumerico) {
  return arrayNumerico.reduce((acc, numero) => acc + numero, 0);
}

console.log(sommaArrayConReduce([1, 2, 3, 4, 5]));

/* ESERCIZIO 6 (map)
  Scrivi una funzione che, dato un array di soli numeri e un numero n come parametri, ritorni un secondo array con tutti i valori del precedente incrementati di n
*/
function incrementaNumeri(arrayNumerico, n) {
  return arrayNumerico.map((numero) => numero + n);
}
console.log(incrementaNumeri([1, 2, 3], 5));
...
console.log(lunghezzaStringhe(["EPICODE", "is", "great"]));
...
function titoliDeiFilm(arrayFilm) {
  return arrayFilm.map((film) => film.Title);
}
...
function filmDelMillennioCorrente(arrayFilm) {
  ...
  return arrayFilm.filter((film) => parseInt(film.Year) >= annoCorrente - 10);
}
